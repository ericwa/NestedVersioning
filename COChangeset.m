#import "COChangeset.h"


@implementation COChangeset
...
- (NSArray *)parents
{
  ...
  // (It would be clearer if COChangeset held pointers to real parent/child objects)
  
  NSMutableArray *result = [NSMutableArray arrayWithCapacity: [_parentNodeUUIDs count]];
  for (ETUUID *uuid in _parentNodeUUIDs)
  {
    COChangeset *node = [_store historyGraphNodeForUUID: uuid];
    if (node)
    {
...
@implementation COChangeset (Private)
