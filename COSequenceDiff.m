...
- (COMergeResult *)mergeWith: (COSequenceDiff *)other
{
	...
	else if (count_oa > 0 && count_ob > 0)
	{
		...
		// FIXME: I think we can eliminate this step...
		...
		for (NSUInteger i=0; i < sorted_count; i++)
		{
			...
			while (i+1 < sorted_count)
			{
				...
				if ([op1 overlaps: op2])
				{
					// FIXME: at this point we know op2 is in the opposite array as op1, because
					// consecutive array elements from one array can't overlap. so we can simplify this code;
					// we only have to scan forward in the "opposite" array
