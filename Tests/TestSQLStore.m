...
@interface TestSQLStore : NSObject <UKTest>
@end

#define FMDBQuickCheck(SomeBool) { if (!(SomeBool)) { NSLog(@"Failure on line %d", __LINE__); return; } }

@implementation TestSQLStore

- (void) dontLoop
{
    NSLog(@"opening 10k db's in wal.");
    
    // takes 6ms to create db and create a table
    
    for (int i=0; i<10000; i++) {
        FMDatabase* db = [[FMDatabase alloc] initWithPath: [NSString stringWithFormat: @"/tmp/tmp%d.db", i]];
        [db open];
